...
## 10. 한 줄 회고
- **김도연**: 정확도만으로는 이진분류 모델 성능을 평가하기 어려워 SMOTE로 데이터 불균형을 조정하고, F1-score를 통해 적은 라벨 데이터에서도 성능을 개선하여 실제 활용 가능한 모델을 만들 수 있다는 것을 배웠습니다. 
- **김영서**: 데이터 전처리부터 모델링까지의 과정에서 불균형 데이터 처리가 얼마나 중요한지 다시금 깨달았고, 다양한 모델을 쌓아 올릴수록 예측 성능이 개선될 가능성이 높다는 점을 실감한 프로젝트였다.
- **김하늘**: 학습자의 이탈 예측을 위해 클릭 수 기반 참여도 분석을 진행하며, 로그 변환과 이상치 탐지를 통해 데이터 분포의 변화를 확인했다. 이 과정에서 클릭 수가 적은 그룹에서 이탈률이 높게 나타나는 패턴을 발견했고, 참여도와 이탈 간의 관계를 이해하는 과정이 모델 성능 개선에 중요하다는 점을 느꼈다.
- **윤환**: 머신러닝을 활용해 학습자 이탈을 예측하고 맞춤형 지원 방안을 제안하며, 데이터 선택과 하이퍼파라미터 튜닝의 중요성을 체감할 수 있었다.
- **조민훈**: 일부의 데이터는 전체의 데이터와 상반되는 값을 가질 수 있다는 점을 실감했으며 데이터의 특성에 따라 모델의 성능차이가 크기때문에 데이터 특성에 관한 이해도의 중요성을 다시금 느꼈다 
